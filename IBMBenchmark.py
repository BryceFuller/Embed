...
from IBMsrc.challenge_submission import compiler_function

# Import submission evaluation and scoring functions
from IBMsrc.challenge_evaluation import evaluate, score

# Possibly useful other helper function
from IBMsrc.challenge_evaluation import qasm_to_dag_circuit, load_coupling, get_layout
...
ex_nr = 5 # examples to add per qubit number. maximum is 10 with the provided circuits
test_circuit_filenames = {}

for i in range(ex_nr):
    test_circuit_filenames['IBMsrc/circuits/random%d_n5_d5.qasm' % i] = get_layout(5)
# for i in range(ex_nr):
#     test_circuit_filenames['IBMsrc/circuits/random%d_n16_d16.qasm' % i] = get_layout(16)
# for i in range(ex_nr):
#     test_circuit_filenames['IBMsrc/circuits/random%d_n20_d20.qasm' % i] = get_layout(20)
