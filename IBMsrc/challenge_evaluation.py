...
def score(compiler_function=None,backend = 'local_qiskit_simulator'):
    ...
    if True:
        for ii in range(ex_nr):
            test_circuit_filenames['IBMsrc/circuits/random%d_n5_d5.qasm' % ii] = load_coupling(maps_q5[ii%len(maps_q5)])["coupling_map"]

    if False:
        for ii in range(ex_nr):
            test_circuit_filenames['IBMsrc/circuits/random%d_n16_d16.qasm' % ii] = load_coupling(maps_q16[ii%len(maps_q16)])["coupling_map"]

    if False:
        for ii in range(ex_nr):
            test_circuit_filenames['IBMsrc/circuits/random%d_n20_d20.qasm' % ii] = load_coupling(maps_q20[ii%len(maps_q20)])["coupling_map"]
...
def evaluate(compiler_function=None, test_circuits=None, verbose=False, backend = 'local_qiskit_simulator'):
    ...
    if compiler_function is not None:
        ...
        res_values_opt = list(map(_compile_circuits, compile_jobs))
        ...
        if verbose:
            compile_jobs = [[name, circuit, 1, _qiskit_compiler, gate_costs] for name, circuit in
                            test_circuits.items()]
            # with Pool(len(compile_jobs)) as job:
            res_values = list(map(_compile_circuits, compile_jobs))
        ...
        # with Pool(len(compile_jobs)) as job:
        res_values = list(map(_prep_sim, compile_jobs))
        ...
        for circuit in qobj_optimized["circuits"]:
            ...
            print("")
            print("Validating circuit", name)
            print("Coupling:", coupling_map)
            ...
            for op in circuit["compiled_circuit"]["operations"]:
                ...
                if op["name"] in ["cx", "CX"] \
                    ...
                    if op["qubits"][0] in coupling_map:
                        ...
                        if not op["qubits"][1] in coupling_map[op["qubits"][0]]:
                            print("Disallowed CNOT:", "->".join(list(map(str,op["qubits"]))))
...
def load_coupling(name):
    ...
    """
    import json
    with open("./IBMsrc/layouts/"+name+".json", 'r') as infile:
...
def _qiskit_compiler(dag_circuit, coupling_map=None, gate_costs=None):
    ...
    compiled_dag, final_layout = swap_mapper(dag_circuit, coupling, initial_layout, trials=40, seed=19)
    # Expand swaps
...
def _compose_qobj(qobj_name, test_circuits,
    ...
    for name, circuit in test_circuits.items():
        ...
        if circuit["dag_"+qobj_name] is not None:
            ...
            # Add circuit
            dag_circuit = circuit["dag_"+qobj_name]
